...
public class RobotContainer {
    ...
    public RobotContainer() {
                ...
                .whileTrue(
                        new RunCommand(() -> drivetrain.drive(
                                -MathUtil.applyDeadband(
                                        Math.copySign(Math.pow(Driver1.getRawAxis(1), 2), Driver1
                                                .getRawAxis(1)),
                                        Operator.kDriveDeadband),
                                -MathUtil
                                        .applyDeadband(
                                                Math.copySign(Math.pow(Driver1.getRawAxis(0), 2),
        ...
        NamedCommands.registerCommand("scoreL1", new SequentialCommandGroup(
                ...
                Commands.waitUntil(() -> elevator.atSetpoint(Elevator.kL1Setpoint))
                        .andThen(() -> pivot.changeSetpoint(PivotConstants.kL1Setpoint))).andThen(
        ...
        NamedCommands.registerCommand("scoreL2", new SequentialCommandGroup(
                ...
                Commands.waitUntil(() -> elevator.atSetpoint(Elevator.kL2Setpoint))
                        .andThen(() -> pivot.changeSetpoint(PivotConstants.kL2Setpoint))).andThen(
                                new RunCommand(() -> roller.setRollerOutput(Roller.kEjectSpeed),
                                        roller).until(() -> !roller.getHasCoral()).andThen(
                                                new InstantCommand(() -> roller.setRollerOutput(0.),
                                                        roller))));
        ...
        NamedCommands.registerCommand("scoreL3", new SequentialCommandGroup(
                ...
                Commands.waitUntil(() -> elevator.atSetpoint(Elevator.kL3Setpoint))
                        .andThen(() -> pivot.changeSetpoint(PivotConstants.kL3Setpoint))).andThen(
                                new RunCommand(() -> roller.setRollerOutput(Roller.kEjectSpeed),
                                        roller).until(() -> !roller.getHasCoral()).andThen(
                                                new InstantCommand(() -> roller.setRollerOutput(0.),
                                                        roller))));
        ...
        NamedCommands.registerCommand("scoreL4", new SequentialCommandGroup(
                ...
                Commands.waitUntil(() -> elevator.atSetpoint(Elevator.kL4Setpoint))
                        .andThen(() -> pivot.changeSetpoint(PivotConstants.kL4Setpoint))).andThen(
                                new RunCommand(() -> roller.setRollerOutput(Roller.kEjectSpeed),
                                        roller).until(() -> !roller.getHasCoral()).andThen(
                                                new InstantCommand(() -> roller.setRollerOutput(0.),
                                                        roller))));
    ...
    private void configureBindings() {
        ...
        Driver1.a().onTrue(
                new InstantCommand(() -> pivot.changeSetpoint(PivotConstants.kCoralSetpoint)));

        // Driver 2

        Driver2.a().onTrue(new SequentialCommandGroup(
                new InstantCommand(() -> pivot.changeSetpoint(PivotConstants.kElevatingSetpoint)),
                new InstantCommand(() -> elevator.ChangeSetpoint(0.0)),
                Commands.waitUntil(() -> elevator.atSetpoint(0.0))
                        .andThen(() -> pivot.changeSetpoint(PivotConstants.kIntakeSetpoint))));
        ...
        Driver2.povUp().onTrue(new SequentialCommandGroup(
                new InstantCommand(() -> pivot.changeSetpoint(PivotConstants.kElevatingSetpoint)),
        ...
        Driver2.rightBumper()
                .whileTrue(new RunCommand(() -> roller.setRollerOutput(Roller.kIntakeSpeed), roller)
                        ...
                        .andThen(new ParallelCommandGroup(new InstantCommand(
                                () -> Driver1.getHID().setRumble(RumbleType.kBothRumble, 1)),
                                new InstantCommand(() -> Driver2.getHID()
                                        .setRumble(RumbleType.kBothRumble, 1)),
                                new RunCommand(
                                        () -> roller.setRollerOutput(Roller.kIntakeFinishSpeed),
                                        roller)).withTimeout(Roller.kIntakeFinishTime)
                                                .andThen(new ParallelCommandGroup(
                                                        new InstantCommand(
        ...
        Driver2.leftBumper()
                .whileTrue(new InstantCommand(
                        () -> pivot.changeSetpoint(PivotConstants.kElevatingSetpoint)).alongWith(
    ...
    public void teleopPeriodic() {
        ...
        // DriverStation.reportError("Big oops: " + e.getMessage(), e.getStackTrace());
        // }
        // SmartDashboard.putNumber("Raw X Speed", -MathUtil.applyDeadband(Driver1.getRawAxis(1),
        // Operator.kDriveDeadband));
        // SmartDashboard.putNumber("Raw Y Speed", -MathUtil.applyDeadband(Driver1.getRawAxis(0),
        // Operator.kDriveDeadband));
    }
    ...
    public static void photonPoseUpdate() {
        ...
        // if (photonPose.getX() >= 0 && photonPose.getX() <= Field.fieldLength && photonPose.getY()
        // >= 0
        ...
        // drivetrain.m_poseEstimator.setVisionMeasurementStdDevs(VecBuilder.fill(xStddev, yStddev,
        // rotStddev));
        // drivetrain.addVisionMeasurement(photonPose.toPose2d(),
        // photonPoseOptional.get().timestampSeconds);
        // }
