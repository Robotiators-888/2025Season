...
public class CMD_ReefAlign extends Command {
  ...
  private final PIDController robotAngleController = new PIDController(0.5, 0.01, 0);
  ...
  public void initialize() {
    ...
    for (int tag : targetTagSet) {
      Pose2d pose = drivetrain.at_field.getTagPose(tag).get().toPose2d();
  ...
  public boolean isFinished() {
    SmartDashboard.putBoolean("LeftAlignCommandComplete!",xController.atSetpoint() && yController.atSetpoint() && robotAngleController.atSetpoint());
